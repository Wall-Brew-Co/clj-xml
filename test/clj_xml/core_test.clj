...
(deftest xml->edn-test
  (testing "Functional correctness"
    (is (match? (sut/xml->edn xml-example) edn-example))
    (is (match? (sut/xml->edn xml-example {:preserve-keys? true}) edn-example-original-keys))
    (is (match? (sut/xml->edn xml-example {:preserve-attrs? true}) edn-example-with-attrs))
    (is (match? (sut/xml->edn xml-example {:preserve-attrs? true :remove-empty-attrs? true}) (update-in edn-example-with-attrs [:test-document :file] dissoc :groups-attrs)))
    (is (match? (sut/xml->edn xml-example {:preserve-keys? true :preserve-attrs? true}) edn-example-with-attrs-and-original-keys))
    (is (match? (sut/xml->edn xml-example {:preserve-keys? true :force-seq? true}) edn-example-original-keys-force-seq))
    (is (nil? (sut/xml->edn nil)))
    (is (nil? (sut/xml->edn :edn)))
    (is (match? (sut/xml->edn {}) {}))
    (is (match? (sut/xml->edn "XML") "XML"))))
...
(deftest insignificant-whitespace-test
  (testing "Corner-cases around embedded insignificant whitespace"
    (let [no-ws (str "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
      ...
      (is (match? {:sometag {:foo "some text", :bar "bla"}}
             (sut/xml-str->edn (str "<sometag><foo>some text</foo><bar>bla</bar></sometag>") {:skip-whitespace true}))))))
...
(deftest force-xml-seq-at-paths-test
  (testing "Parsed XML can coerce child nodes to collections"
    (let [nested-data {:a {:b [1 2 3] :c {:d "e"}}}]
      (is (match? (sut/force-xml-seq-at-paths nested-data [[:a :b sut/first-child] [:a :b sut/last-child] [:a]])
             {:a [{:b [[1] 2 [3]] :c {:d "e"}}]}))
      (is (match? (sut/force-xml-seq-at-paths nested-data [[:a] [:a sut/first-child :b]])
             {:a [{:b [[1 2 3]] :c {:d "e"}}]})))))


(deftest xml-sequence-coercion-test
  (testing "parsed XML can be coerced"
    (is (match? (sut/xml->edn' xml-example {:force-seq-for-paths [[:test-document :file :segments sut/every-child]
                                                                  [:test-document]]})
